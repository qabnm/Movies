...
class PrivacyDialogFragment(private val listener:OnDialogBtnClickListener?) : DialogFragment(){
    override fun onCreateView(
        ...
        val layoutView = inflater.inflate(R.layout.dialog_privacy, container, false)
    ...
    private fun initViews(view: View) {
        ...
        val spannableString = SpannableString("感谢您选择多多影视大全App！我们非常重视您的个人信息和隐私保护。为了更好的保障您的个人权益，" +
        ...
        spannableString.apply {
            setSpan(ForegroundColorSpan(ContextCompat.getColor(requireContext(),R.color.color557CE7)),65,77,Spanned.SPAN_INCLUSIVE_EXCLUSIVE)
            setSpan(ForegroundColorSpan(ContextCompat.getColor(requireContext(),R.color.color557CE7)),78,90,Spanned.SPAN_INCLUSIVE_EXCLUSIVE)
            setSpan(BackgroundColorSpan(ContextCompat.getColor(requireContext(),R.color.colorFFFFFF)),65,77,Spanned.SPAN_INCLUSIVE_EXCLUSIVE)
            setSpan(BackgroundColorSpan(ContextCompat.getColor(requireContext(),R.color.colorFFFFFF)),78,90,Spanned.SPAN_INCLUSIVE_EXCLUSIVE)
        ...
        tvCancel.setOnClickListener { listener?.onDialogCancelClick() }
        tvSure.setOnClickListener { listener?.onDialogSureClick() }
    }
