...
import com.duoduovv.download.tool.RoomClient
...
class DownloadScope(
     ...
     * 获取downloadInfo
     */
    fun downloadInfo(): DownloadInfo? = downloadData.value
...
//            downloadInfo.fileName = UrlUtils.getUrlFileName(downloadInfo.url)
    ...
    private fun change(status: Int) = launch(Dispatchers.Main) {
        ...
        withContext(Dispatchers.IO) {
            RoomClient.instance.getDataBase(context).downloadDao().insertOrReplace(downloadInfo)
        }
    ...
    fun remove() = launch(Dispatchers.Main) {
        ...
        withContext(Dispatchers.IO) {
            downloadInfo?.let {
                RoomClient.instance.getDataBase(context).downloadDao().delete(it)
                //同时删除已下载的文件
                it.path?.let { path ->
                    val file = File(path)
    ...
    init {
        launch(Dispatchers.Main) {
            val downloadInfoDeferred = async(Dispatchers.IO) {
                RoomClient.instance.getDataBase(context).downloadDao().queryByUrl(url)
            }
